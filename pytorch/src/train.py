...
def transfer_classification(config):
    ...
    for i in range(config["num_iterations"]):
        if i % 10 == 0:
            print("[>>>>>>>>>>> iteration: %d|%d]" % (i, config["num_iterations"]))
        ...
        print("total loss:{:.6f}, transfer loss: {:.6f}, classifier loss: {:.6f}"
              .format(total_loss.data[0], transfer_loss.data[0], classifier_loss.data[0]))
        total_loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_list_file', type=str, default='/home/wk/project/_data/office31/', )
    ...
    config["data"] = [
        {"name": "source", "type": "image", "list_path": {"train": args.data_list_file + args.source + "_list.txt"},
         "batch_size": {"train": 36, "test": 4}},
        {"name": "target", "type": "image", "list_path": {"train": args.data_list_file + args.target + "_list.txt"},
         "batch_size": {"train": 36, "test": 4}}]
